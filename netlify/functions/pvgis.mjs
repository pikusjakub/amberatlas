...
export default async (request) => {
  ...
  // Broad Morocco bounds. They also prevent this endpoint from becoming
  // an open global proxy for arbitrary PVGIS requests.
  if (
    !Number.isFinite(lat) ||
    !Number.isFinite(lon) ||
    lat < 20 ||
  ...
  try {
    const response = await fetch(pvgisUrl, {
      headers: {
        ...
        "user-agent": "AmberAtlasSolarCalculator/2.0"
      },
    ...
    if (!response.ok) {
      return json(
        { error: "PVGIS service unavailable", status: response.status },
        502
      );
    }
    ...
    const message =
      error?.name === "AbortError"
        ? "PVGIS request timeout"
        : "PVGIS request failed";
